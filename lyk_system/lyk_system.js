...
function analysis(data){    
    ...
    var point_length=data.point.length;
    ...
    for(var i = 0; i < data.compute.length; i++){
        ...
        if(i==2){
            if(obj1.status == 1){
                obj1.symbol = 'image://../images/jhjz01.png';
            }
            else if (obj1.status == 0) {
                obj1.symbol = 'image://../images/jhjz02.png';
            }
        }
        end_obj1.push(obj1);    
    };
    ...
    for(var k = 0; k <data.switchs.length; k++){
        ...
        obj3.status=data.switchs[k].status;
         if(obj3.status==1){
             obj3.symbol = 'image://../images/image_fhq_1.png';
         }else if (obj3.status==0){
             obj3.symbol = 'image://../images/image_fhq_2.png';
         }
        end_obj3.push(obj3);
    };
    ...
    for(var k = 0; k <data.database.length; k++){
        var obj4 = {name:'',x:0,y:450,symbol:'',symbolSize: [50,50]};
    ...
    for(var j = 0; j < data.storage.length; j++){
        var obj2 = {name:'',obj_id:'',x:0,y:150,symbol:'',status:'',symbolSize: [50,50]};//storage
...
//磁盘阵列
    for(var j = 0; j < data.cpzl.length; j++){
        var obj5 = {name:'',obj_id:'',x:0,y:900,symbol:'',status:'',symbolSize: [60,30]};//storage
    ...
    for(var j = 0; j < data.sujuku.length; j++){
        var obj6 = {name:'',obj_id:'',x:0,y:750,symbol:'',status:'',symbolSize: [50,50]};//storage
    ...
    for(var j = 0; j < data.point.length; j++){
        ...
        obj2.name = data.point[j].name;
        obj2.obj_id = data.point[j].id;
        obj2.value = data.point[j].type;
        obj2.msg=data.point[j].msg;
        // obj2.x = 150*(j+1);
        if(point_length == 1){
            obj2.x = 1200/2-(113/2);
        }else{
            obj2.x = 1300/(point_length+1)*(j+1)-(50/2);

        }
        obj2.status=data.point[j].status;
        obj2.kind = data.point[j].kind;
        if(obj2.kind == 1){
            if(obj2.status == 1){
                obj2.symbol = 'image://../images/image001_1.png';
            }else if(obj2.status == 0){
        ...
        if(j==9){
            if(obj2.status == 1){
                obj2.symbol = 'image://../images/key01.png';
            }else if(obj2.status == 0){
                obj2.symbol = 'image://../images/key02.png';
    ...
    //在push上面几个数组的时候，要将data.storage的节点数组放在data.compute之前,为什么还是清楚
     end_obj.push(end_obj4,mix_compute_attr,end_obj3,end_obj2,end_obj5,end_obj6,end_obj7);
    var attr = [];
